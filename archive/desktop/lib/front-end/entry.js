...
var init = tandem_front_end_1.setup(function () {
...
document.body.addEventListener("click", function (event) {
    if (event.target.tagName === "A" ||
        ...
        child_process_1.exec("open " + href);
    }
});
...
setTimeout(init, 500, {
    ...
    unloaders: [],
    sourceNodeInspector: tandem_front_end_1.createRootInspectorNode(),
...
function openPreview(frame) {
    ...
    return __generator(this, function (_a) {
        switch (_a.label) {
            ...
            case 1:
                state = _a.sent();
                sourceNode = paperclip_1.getSyntheticSourceNode(paperclip_1.getSyntheticNodeById(frame.syntheticContentNodeId, state.documents), state.graph);
                dep = paperclip_1.getPCNodeDependency(sourceNode.id, state.graph);
                child_process_1.exec("open http://" + query.previewHost + "/preview.html?contentNodeId=" + sourceNode.id + "\\&entryPath=" + encodeURIComponent(tandem_common_1.stripProtocol(dep.uri)));
...
function readDirectory(dirUri) {
    ...
    return __generator(this, function (_a) {
        switch (_a.label) {
            case 0:
                dir = tandem_common_1.stripProtocol(dirUri);
            ...
            case 1:
                ...
                return [2 /*return*/, dirBasenames.map(function (basename) {
                        var fullPath = tandem_common_1.normalizeFilePath(path.join(dir, basename));
                        var uri = tandem_common_1.addProtocol(tandem_common_1.FILE_PROTOCOL, fullPath);
                        if (fs.lstatSync(fullPath).isDirectory()) {
                            return tandem_common_1.createDirectory(uri);
                        }
                        else {
                            return tandem_common_1.createFile(uri);
                        }
                    })];
        }
    });
...
function deleteFile(uri) {
    var path;
    return __generator(this, function (_a) {
        path = tandem_common_1.stripProtocol(uri);
        fsa.removeSync(path);
        return [2 /*return*/];
    });
}
...
function readFile(uri) {
    return Promise.resolve({
        content: fs.readFileSync(tandem_common_1.stripProtocol(uri)),
