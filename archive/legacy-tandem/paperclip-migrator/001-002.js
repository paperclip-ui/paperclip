...
module.exports = (module) => {
  ...
  const getComponentInstanceId = (componentName) => {
    const component = module.children.find(
      (component) => component.attributes.core.name === componentName
    );
  ...
  const mapVisibleNode = (node) => {
    switch (node.name) {
      ...
      case "text":
        return mapText(node);
      default:
        return mapComponentInstance(node);
    }
  };
  ...
  const mapElement = ({ id, name, attributes, children }) => ({
    ...
    attributes: {},
    is:
      attributes.core.nativeType && attributes.core.nativeType !== "element"
