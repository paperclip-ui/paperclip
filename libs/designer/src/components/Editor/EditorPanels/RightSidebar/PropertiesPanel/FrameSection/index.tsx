...
export const FrameSection = ({ bounds }: FrameSectionProps) => {
    return <sidebarStyles.SidebarSection>
        <sidebarStyles.SidebarPanelHeader>Frame</sidebarStyles.SidebarPanelHeader>
        <sidebarStyles.SidebarPanelContent>
            <inputStyles.Fields>
            {inputs.map(input => {
                return <PropInput key={input.name} name={input.name} bounds={bounds} />
            })}
            </inputStyles.Fields>
        </sidebarStyles.SidebarPanelContent>
...
const PropInput = ({ name, bounds }: PropInputProps) => {   
    ...
    const onChange = (value: string) => {
        dispatch({ type: "designer/boundsChanged", payload: { newBounds: { ...bounds, [name]: parseInt(value) } }});
    };

    return <inputStyles.Field name={name} input={<TextInput value={bounds[name]} onChange={onChange} />} />
};