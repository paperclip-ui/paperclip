...
use super::core::{Dispatcher, Machine, Reducer, Engine};
...
impl Engine<Event, State> for AppEngine<Event> {
  fn on_event(&self, event: &Event, state: &State, prev_state: &State) {
    ...
    if matches!(event, Event::Inc(_)) {
      self.tx.send(Event::SomeSideEffect(state.count + prev_state.count)).unwrap();
    }
  }
}
...
pub async fn can_create_a_simple_machine() {
    let machine = Machine::new(State { count: 0, sum: 0 }, |tx| {
      AppEngine { tx }
    });
    machine.start().await;
    ...
    async {
      assert_eq!(machine.get_state().lock().unwrap().clone(), State { count: 1, sum: 0 });
    }.await;
