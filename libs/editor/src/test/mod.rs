use super::{Mutation, ApplyMutation, AppendChild};
...
macro_rules! case {
    ($name: ident, $mock_files: expr, $edit: expr, $expected_mock_files: expr) => {
        ...
        pub fn $name() {
          ...
          $edit.apply(&mut graph);

          let edited_docs = graph.dependencies.iter().map(|(path, dep)| {
            (path.to_string(), pc::serializer::serialize(&dep.document))
...
case! {
  ...
  Mutation::AppendChild(AppendChild {
    ...
    node: ast::pc::node::Inner::Element(ast::pc::Element {
...
case! {
  ...
  Mutation::AppendChild(AppendChild {
    parent_id: "80f4925f-2".to_string(),
    node: ast::pc::node::Inner::Text(ast::pc::TextNode {
      id: "something".to_string(),
      name: None,
