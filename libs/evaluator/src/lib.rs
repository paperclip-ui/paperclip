...
pub async fn evaluate(path: String, graph: JsValue) -> JsValue {
  ...
  let html = html::evaluator::evaluate(&path, &graph, &JsFileResolver::default(), html::context::Options {
    include_components: true
  }).await.unwrap();

  let css = css::evaluator::evaluate(&path, &graph, &JsFileResolver::default()).await.unwrap();
