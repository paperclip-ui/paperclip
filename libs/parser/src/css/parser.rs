...
pub fn parse_style_declaration_with_string_scanner<'src, 'scanner, 'idgenerator>(
    ...
    Ok(parse_decl_value(&mut context)?.wrap())
}
...
fn parse_style_declaration(
    ...
    let value = parse_comma_list(context)?.wrap();
...
fn parse_comma_list(
    ...
    Ok(if matches!(context.curr_token, Some(Token::Comma)) {
        let mut items = vec![first.wrap()];
        while matches!(context.curr_token, Some(Token::Comma)) {
            context.next_token()?;
            context.skip(is_superfluous)?;
            items.push(parse_spaced_list(context)?.wrap());
        }
        let end = context.curr_u16pos.clone();
        ast::declaration_value::Value::CommaList(ast::CommaList {
            id: context.next_id(),
            range: Some(Range::new(start, end)),
            items,
        })
...
fn parse_spaced_list(
    ...
    Ok(if matches!(context.curr_token, Some(Token::Space(_))) {
        ...
        while matches!(context.curr_token, Some(Token::Space(_))) {
            context.next_token()?;
            items.push(parse_arithmetic(context)?.wrap());
        }
        let end = context.curr_u16pos.clone();
        ast::declaration_value::Value::SpacedList(ast::SpacedList {
            id: context.next_id(),
            range: Some(Range::new(start, end)),
            items,
        })
...
fn parse_arithmetic(
    ...
    Ok(if let Some(operator) = operator_option {
        ...
        let end = context.curr_u16pos.clone();
        ast::declaration_value::Value::Arithmetic(Box::new(ast::Arithmetic {
            id: context.next_id(),
            range: Some(Range::new(start, end)),
            left: Some(Box::new(left.wrap())),
            right: Some(Box::new(right.wrap())),
            operator,
        }))
    } else {
        left
...
fn parse_keyword(
    ...
    Ok(if context.peek(1) == &Some(Token::ParenOpen) {
        ...
        ast::declaration_value::Value::Reference(parse_reference(context)?)
    })
}
...
fn parse_string(
    ...
    if let Some(Token::String(value)) = context.curr_token {
        ...
        context.next_token()?;
        let end = context.curr_u16pos.clone();
        Ok(ast::declaration_value::Value::Str(base_ast::Str {
            id: context.next_id(),
            range: Some(Range::new(start, end)),
...
fn parse_hex_color(
    ...
    Ok(ast::declaration_value::Value::HexColor(ast::HexColor {
        id: context.next_id(),
        range: Some(Range::new(start, end)),
        value,
    }))
...
fn parse_call(context: &mut ParserContext) -> Result<ast::FunctionCall, err::ParserError> {
    ...
    context.next_token()?;
    context.next_token()?; // eat (
    let arguments = parse_comma_list(context)?.wrap();
    context.next_token()?;
    let end = context.curr_u16pos.clone();
    Ok(ast::FunctionCall {
        id: context.next_id(),
...
fn parse_decl_number(
    ...
    if let Some(Token::Number(value)) = context.curr_token {
        ...
        Ok(if let Some(unit) = unit_option {
            ...
            ast::declaration_value::Value::Measurement(ast::Measurement {
                id: context.next_id(),
            ...
            ast::declaration_value::Value::Number(base_ast::Number {
                id: context.next_id(),
                range: Some(Range::new(start, context.curr_u16pos.clone())),
                value,
            })
