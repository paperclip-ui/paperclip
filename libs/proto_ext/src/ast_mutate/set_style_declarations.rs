...
impl<'expr> Visitor<Vec<MutationResult>> for EditContext<'expr, SetStyleDeclarations> {
    fn visit_style(&mut self, expr: &mut ast::pc::Style) -> VisitorResult<Vec<MutationResult>> {
        if expr.get_id() == self.mutation.expression_id {
            let new_style = parse_style(&mutation_to_style(&self.mutation, &self.dependency), &expr.checksum());
    ...
    fn visit_document(&mut self, doc: &mut ast::pc::Document) -> VisitorResult<Vec<MutationResult>> {
        ...
        for (ns, (_old_ns, path, is_new)) in get_dep_imports(&self.mutation, &self.dependency) {
            if is_new {
                ...
                println!("{} {}", path, Path::new(&self.dependency.path).parent().unwrap().to_str().unwrap());
                let relative = diff_paths(&path, Path::new(&self.dependency.path).parent().unwrap()).unwrap();
                ...
                let new_imp_doc = parse_pc(format!("import \"{}\" as {}", relative, ns).as_str(), doc.checksum().as_str()).unwrap();
    ...
    fn visit_element(&mut self, expr: &mut ast::pc::Element) -> VisitorResult<Vec<MutationResult>> {
        if expr.get_id() == &self.mutation.expression_id {
            let new_style: ast::pc::Style = parse_style(&mutation_to_style(&self.mutation, &self.dependency), &expr.checksum());
...
fn get_dep_imports(mutation: &SetStyleDeclarations, dep: &Dependency) -> HashMap<String, (String, String, bool)> {
    ...
    for decl in &mutation.declarations {
        for (key, path) in &decl.imports {
            let existing_import = dep.imports.iter().find_map(|(relative, imp_path)| {
                if imp_path == path {
                    let ns = dep.document.as_ref().unwrap().get_imports().iter().find(|imp| {
                        &imp.path == relative
                    }).unwrap().namespace.to_string();
            ...
            if let Some(ns) = existing_import {
                ...
                let mut inc = 1;
                loop {
                    if matches!(dep.imports.iter().find(|(existing_ns, _path)| existing_ns == &&ns), None) {
                        break;
                    }
                    ns = format!("imp{}", inc);
                    inc += 1;
