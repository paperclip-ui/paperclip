...
use paperclip_proto::ast::pc::{
    component_body_item, node, Component, Element, Insert, Render, SimpleExpression,
};
use paperclip_proto::ast::{
    all::Expression,
...
pub fn get_instance_component<'a>(tag_name: &str, namespace: Option<String>, owner: &'a Dependency, graph: &'a Graph) -> Option<&'a Component> {
   ...
   dep.document.as_ref().expect("Document must exist").get_component_by_name(tag_name)
}
