...
export type InspectorRoot =
  {} & InspectorTreeBaseNode<InspectorTreeNodeName.ROOT>;

export type InspectorSourceRep =
  {} & InspectorTreeBaseNode<InspectorTreeNodeName.SOURCE_REP>;

export type InspectorShadow =
  {} & InspectorTreeBaseNode<InspectorTreeNodeName.SHADOW>;
    ...
    for (const inspectorNodeId of targetInspectorNodeInstanceIds) {
      ...
      switch (ot.type) {
        ...
        case TreeNodeOperationalTransformType.REMOVE_CHILD: {
          ...
          const inspectorChildIndex =
            newInspectorNode.children.indexOf(inspectorChild);
