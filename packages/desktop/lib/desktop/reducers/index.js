...
var rootReducer = function (state, action) {
    switch (action.type) {
        case actions_1.TD_PROJECT_FILE_PICKED: {
            ...
            return __assign(__assign({}, state), { tdProjectPath: (0, tandem_common_1.normalizeFilePath)(filePath) });
        }
        case actions_1.LOCAL_FILE_LOADED: {
            var path = action.path;
            if (/.tdproject/.test(path)) {
                return __assign(__assign({}, state), { tdProjectPath: (0, tandem_common_1.normalizeFilePath)(path) });
            }
            return state;
        }
