...
export default compose(pure, withState(`hsla`, `setHSLA`, ({ value = "#000", swatchOptionGroups = EMPTY_ARRAY }) => {
    ...
    componentWillUpdate(props) {
        if (this.props.value !== props.value) {
            const rgba = parseRGBA(maybeConvertSwatchValueToColor(props.value, props.swatchOptionGroups));
    ...
    const colorChangeCallback = updater => ({ onChange, hsla, setHSLA }) => throttle(rgba => {
        setHSLA((hsla = updater(rgba, hsla)));
        if (onChange) {
            onChange(stringifyRgba(hslaToRgba(hsla)));
        }
    }, CHANGE_THROTTLE_MS);
    const colorChangeCompleteCallback = updater => ({ onChangeComplete, hsla, setHSLA }) => rgba => {
        setHSLA((hsla = updater(rgba, hsla)));
        if (onChangeComplete) {
    ...
    var { value, hsla, onChange, onChangeComplete, onRGBAInputChange, onRGBAInputChangeComplete, onColorSwatchChange, onHSLChange, onHSLChangeComplete, onSpectrumChange, onSpectrumChangeComplete, swatchOptionGroups = EMPTY_ARRAY, onOpacityChange, onOpacityChangeComplete } = _a, rest = __rest(_a, ["value", "hsla", "onChange", "onChangeComplete", "onRGBAInputChange", "onRGBAInputChangeComplete", "onColorSwatchChange", "onHSLChange", "onHSLChangeComplete", "onSpectrumChange", "onSpectrumChangeComplete", "swatchOptionGroups", "onOpacityChange", "onOpacityChangeComplete"]);
...
const stringifyRgba = ([r, g, b, a]) => `rgba(${r}, ${g}, ${b}, ${a})`;
const hslDrawer = memoize((h) => (canvas, width = 100, height = 100) => {
    const hv = h * 360;
...
const hslPointer = (hsl, width, height) => {
...
const opacityPointer = ([h, s, l, a], width, height) => {
    return {
        left: width * a
    };
};
const huePointer = ([h], width, height) => {
    return {
        left: (width * (h * 360)) / 360
    };
};
const hueDrawer = (canvas, width, height) => {
    const ctx = canvas.getContext("2d");
    canvas.width = width;
    canvas.height = height;
...
const opacityDrawer = memoize((h) => (canvas, width, height) => {
...
const updateOpacity = (rgba, [h, s, l]) => {
    const l2 = rgbaToHsla(rgba)[2];
    return [h, s, l, lToA(l2)];
};
const lToA = l => Number((1 - Math.min(0.5, Math.max(0, l - 0.5)) / 0.5).toFixed(2));
const updateHue = (rgba, [, s, l, a]) => [
    rgbaToHsla(rgba)[0],
    s,
    l,
    a
];
const updateHSLA = (rgba, [h, , , a]) => {
...
const hslToHsv = (hsl) => rgbToHsv(hslaToRgba(hsl));
const parseRGBA = memoize((value) => {
