import { parseCSSBackroundValue, CSSBackgroundType } from "./state";
import { expect } from "chai";
describe(__filename + "#", () => {
    it("can parse a simple linear-gradient as a red solid color", () => {
        const images = parseCSSBackroundValue("linear-gradient(red, red)");
    ...
    it("can parse a simple linear-gradient as a rgba solid color", () => {
        const images = parseCSSBackroundValue("linear-gradient(rgba(0, 0, 0, 1), rgba(0, 0, 0, 1))");
        expect(images.length).to.eql(1);
        expect(images[0].type).to.eql(CSSBackgroundType.SOLID);
        expect(images[0].color).to.eql("rgba(0, 0, 0, 1)");
    });
    it("can parse a simple linear-gradient as rgba", () => {
        const images = parseCSSBackroundValue("linear-gradient(0deg, rgba(0, 0, 0, 1) 10%, rgba(0, 0, 0, 0.5) 11%)");
    ...
    it("can parse a simple background image", () => {
        const images = parseCSSBackroundValue("url(abcd.png)");
        expect(images.length).to.eql(1);
    ...
    it("can parse a simple background image with spaces", () => {
        const images = parseCSSBackroundValue("url('a b c d.png')");
        expect(images.length).to.eql(1);
        const image = images[0];
        expect(image.type).to.eql(CSSBackgroundType.IMAGE);
    ...
    it("can parse multiple background images", () => {
        const images = parseCSSBackroundValue("linear-gradient(rgba(0, 0, 0, 1), rgba(0, 0, 0, 1)), linear-gradient(0deg, rgba(0, 0, 0, 1) 10%, rgba(0, 0, 0, 0.5) 11%)");
