...
exports.hookRemotePCRuntime =
  exports.createRemotePCRuntime =
  exports.createLocalPCRuntime =
  exports.RemotePCRuntime =
    void 0;
...
var LocalPCRuntime = /** @class */ (function (_super) {
  ...
  LocalPCRuntime.prototype._evaluateNow = function () {
    ...
    var newSyntheticDocuments = (0, evaluate_1.evaluateDependencyGraph)(
      this._info.graph,
    ...
    for (var uri in newSyntheticDocuments) {
      ...
      if (prevSyntheticDocument) {
        var ots = (0, ot_1.diffTreeNode)(
          prevSyntheticDocument,
          newSyntheticDocument
        );
        if (ots.length) {
          prevSyntheticDocument = documentMap[uri] = (0, ot_1.patchTreeNode)(
            ots,
            prevSyntheticDocument
          );
          diffs[uri] = ots;
...
var RemotePCRuntime = /** @class */ (function (_super) {
  ...
  function RemotePCRuntime(_remote, info) {
    ...
    _this._onRemoteMessage = function (event) {
      ...
      if (type === "fetchInfo") {
        ...
        var newDocuments = payload[0],
          diffs = payload[1],
          deletedDocumentUris = payload[2],
          timestamp = payload[3];
        _this._syntheticDocuments = __assign(
          __assign({}, _this._syntheticDocuments),
          newDocuments
        );
        for (var uri in diffs) {
          _this._syntheticDocuments[uri] = (0, ot_1.patchTreeNode)(
            diffs[uri],
            _this._syntheticDocuments[uri]
          );
        }
        for (
          var _i = 0, deletedDocumentUris_1 = deletedDocumentUris;
          _i < deletedDocumentUris_1.length;
          _i++
        ) {
  ...
  RemotePCRuntime.prototype.setInfo = function (value, timestamp) {
    ...
    if (
      ...
      !(0, lodash_1.isEqual)(
        value.variants,
        (oldInfo || tandem_common_1.EMPTY_OBJECT).variants
      )
    ) {
...
var createRemotePCRuntime = function (remote, info) {
  return new RemotePCRuntime(remote, info);
};
  ...
  for (var uri in changes) {
    ...
    if (Array.isArray(change)) {
      newGraph[uri] = change.length
        ? (0, dsl_1.createPCDependency)(
            uri,
            (0, ot_1.patchTreeNode)(change, oldGraph[uri].content)
          )
        : oldGraph[uri];
    } else {
...
console.log("OK");
var hookRemotePCRuntime = function (localRuntime, remote) {
    ...
    return __generator(this, function (_a) {
      ...
      remote.addEventListener("message", function (event) {
        ...
        if (type === "fetchAllSyntheticDocuments") {
          ...
          var localInfo =
            localRuntime.getInfo() || tandem_common_1.EMPTY_OBJECT;
          localRuntime.setInfo(
            __assign(__assign({}, localInfo), {
