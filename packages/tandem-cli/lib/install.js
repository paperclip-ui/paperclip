...
var url = "https://github.com/tandemcode/tandem/releases/download/v"
  .concat(distVersion, "/")
  .concat(archName, ".zip");
var start = function (cwd) {
  return function (_a) {
    ...
    return download(
      url,
      force,
      path.join(constants_1.TMP_APP_ROOT_DIR, fileName),
      constants_1.TMP_APP_DIR
    );
  };
};
exports.start = start;
    ...
    request
      ...
      .on("response", function (response) {
        ...
        var bar = new ProgressBar(
          "Downloading Tandem ".concat(distVersion, " [:bar]"),
...
function install(filePath, unzipPath) {
  return new Promise(function (resolve, reject) {
    ...
    yauzl.open(filePath, function (err, zipFile) {
      var bar = new ProgressBar(
        "Unzipping Tandem ".concat(distVersion, " [:bar]"),
        {
          width: 40,
